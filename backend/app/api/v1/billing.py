...
from backend.app.core.identity_security import ProductCurrentUser, get_product_user, require_product_admin
...
def create_checkout_session(payload: CreateCheckoutRequest, request: Request, idempotency_key: str = Header(..., alias="Idempotency-Key", max_length=255), current: ProductCurrentUser = Depends(get_product_user), db: Session = Depends(get_db)):
...
def create_portal_session(request: Request, idempotency_key: str = Header(..., alias="Idempotency-Key", max_length=255), current: ProductCurrentUser = Depends(get_product_user), db: Session = Depends(get_db)):
...
def list_catalog_versions(request: Request, current: ProductCurrentUser = Depends(require_product_admin), db: Session = Depends(get_db)):
...
def create_catalog_version(payload: CreateBillingCatalogVersionRequest, request: Request, idempotency_key: str = Header(..., alias="Idempotency-Key", max_length=255), current: ProductCurrentUser = Depends(require_product_admin), db: Session = Depends(get_db)):
...
def activate_catalog_version(catalog_version_id: str, payload: ActivateBillingCatalogRequest, request: Request, idempotency_key: str = Header(..., alias="Idempotency-Key", max_length=255), current: ProductCurrentUser = Depends(require_product_admin), db: Session = Depends(get_db)):
    service = ProductBillingService(db)
...
def set_feature_credit_price(feature_key: str, payload: UpdateFeatureCreditPriceRequest, request: Request, idempotency_key: str = Header(..., alias="Idempotency-Key", max_length=255), current: ProductCurrentUser = Depends(require_product_admin), db: Session = Depends(get_db)):
    service = ProductBillingService(db)
...
def update_signup_trial_policy(payload: UpdateSignupTrialPolicyRequest, request: Request, idempotency_key: str = Header(..., alias="Idempotency-Key", max_length=255), current: ProductCurrentUser = Depends(require_product_admin), db: Session = Depends(get_db)):
    service = ProductBillingService(db)
...
def create_credit_adjustment(payload: CreditAdjustmentRequest, request: Request, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key", max_length=255), current: ProductCurrentUser = Depends(require_product_admin), db: Session = Depends(get_db)):

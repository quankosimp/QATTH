...
| FR-CV-001..009 | scans, attempts, drafts, serialized confirm, DB-immutable versions, analysis, retry/archive; CV domain tables | <code>test_product_cv_contract.py</code>, <code>test_domain_history_invariants.py</code> | Implemented |
| FR-INT-001..002, FR-INT-004..008 | interview/session token/events/report/retry/feedback; aggregate locking, event dedup/sequence and DB-immutable timeline | <code>test_product_interview_contract.py</code>, <code>test_domain_history_invariants.py</code> | Implemented |
...
| FR-JOB-001, FR-JOB-003..005, FR-JOB-010..012 | allowlisted source ingest with redirect re-check, postings/snapshots/interactions/applications/moderation and DB-protected application history | <code>test_product_job_search_contract.py</code>, <code>test_job_search.py</code>, <code>test_job_ingest_policy.py</code>, <code>test_append_only_product_events.py</code> | Implemented |
...
| FR-PRIV-001..003 | encrypted export with token/download audit, checkpointed deletion/consent workflows and per-artifact retryable cleanup; privacy_requests/events | <code>test_product_privacy_contract.py</code>, <code>test_privacy_artifact_retention.py</code> | Implemented; production backup expiry is NFR evidence |
| FR-ADMIN-001..005 | masked/audited resource and user search, immutable model config with quality-gated canary/rollback, durable idempotent background-job retry, source/moderation, credit dual-control | <code>test_product_admin_ops_contract.py</code>, <code>test_model_quality_governance.py</code>, <code>test_admin_job_retry_outbox.py</code>, <code>test_product_billing_contract.py</code> | Implemented |
| FR-OPS-001..003 | liveness; readiness gates DB/schema/Redis/queue/storage/malware; persisted async-run correlation propagated through Celery; atomic Redis IP/principal/action quotas and AI-cost policy | <code>test_contract.py</code>, <code>test_product_health_readiness.py</code>, <code>test_async_run_correlation.py</code>, <code>test_provider_resilience.py</code>, <code>test_distributed_rate_limit.py</code> | Implemented; multi-instance/load evidence remains an NFR gate |
...
| NFR-PRIV-005 | scheduled per-artifact cleanup with retry, immutable audit and Prometheus backlog/age metrics | Partial: managed backup expiration evidence and staging cleanup drill required |
| NFR-OBS-001..004, NFR-OBS-006 | correlation; API/provider plus DB pool, queue/dispatch age, interview concurrency and job freshness Prometheus metrics; JSON events with trace/request IDs and safe error code; W3C propagation and safe OTLP spans across FastAPI/SQLAlchemy/Celery/provider; immutable privileged/privacy audit | Implemented in backend; <code>test_async_run_correlation.py</code>, <code>test_product_runtime_metrics.py</code>, <code>test_structured_logging.py</code>, <code>test_provider_resilience.py</code>, <code>test_telemetry_contract.py</code>, admin/privacy tests; collector acceptance remains environment evidence |
| NFR-OBS-005 | alert platform handoff | Partial/evidence pending: dashboard, alert and runbook links |
...
| NFR-AI-002..003, NFR-AI-006 | immutable eval-report ingestion with dataset checksum/sample count, purpose-specific server thresholds, one-baseline/one-canary invariant, deterministic allocation and version rollback | Implemented in backend; <code>test_model_quality_governance.py</code>; representative bilingual datasets, expert annotations and provider eval runs remain release evidence |
| NFR-MNT-001..006 | adapters, OpenAPI sync, offline tests, production fail-fast config with legacy/demo runtime disabled, schema-head pin, graceful shutdown and Docker API/worker/beat/migration contract | Implemented; <code>test_openapi_contract_sync.py</code>, <code>test_production_config.py</code>, <code>test_database_revision_contract.py</code> and <code>test_container_contract.py</code> |
